...
- 连接中断后按 `reconnect_delay_ms` 重连；连接保持期间每 30 秒发送协议心跳；平台输入异常只返回连接器错误。 连接器同时暴露共享健康状态，服务会投影“连接中/已连接/断线重连/失败”到桌面诊断。
...
真实连接器的账号权限、Cookie 生命周期和平台协议属于部署边界；生产接入前仍应在目标账号和网络环境补充真实连接契约测试；当前代码已用本地 fake WebSocket 覆盖握手、事件接收、断线重连和重复事件 ID，再由 EventBus 验证重复投递被丢弃，同时覆盖 zlib、嵌套包、包大小上限和限流边界。Brotli 需要显式启用编解码器后再开放。
## 服务端接入（可视化向导或配置文件）
