...
连接保持后，桌面端每约 2 秒同步事件，并每约 2 秒刷新一次组件健康状态；模型服务在桌面启动后才启动或停止时，健康面板也会更新。急停状态优先保留，不会因为重连显示“假连接”。 OBS 断线后服务会在动作或健康探针触发受控重连，桌面会显示重连中/失败/恢复详情。 Bilibili 启用后也会显示连接中、已连接、断线和重连失败详情；关闭时不显示误导性的红色平台状态。
