...
- 桌面端的只读状态轮询与用户命令独立执行，状态查询无响应不会挡住打断和急停请求。用户命令仍按提交顺序发送；服务本身不可用时无法保证命令执行，界面会报告失败。
...
`persona` 读取当前角色快照；`set_persona` 经过版本/字段校验后更新 Runtime 人格，并广播 `persona_changed` 事件。同一 `profile_id` 的编辑保留上下文；不同 ID 会停止旧音频、切换记忆范围并清空短期对话。记忆适配器不支持身份隔离时拒绝切换，不会静默共用旧记忆。活动回合期间切换会失败，避免一半回复使用旧人格、一半回复使用新人格。并发人格更新串行提交，读取人格会等待正在进行的更新完成。`submit` 返回 accepted 后异步执行。客户端通过 `status` 获取最新只读快照，通过
