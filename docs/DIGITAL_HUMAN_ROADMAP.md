...
当前已落地图片外形包与人格角色包，两者可分别导入；声音、行为和场景还没有包管理与组合编辑，独立网页/VRM 载体尚未实现。角色间记忆已隔离，仍需补交互对象隔离、记忆来源、纠正和关系连续性。
...
身份与记忆切换已实现：活动回合拒绝切换，空闲时切换会停止剩余音频、清空短期上下文并选择持久记忆范围。角色包保存、导入与预览应用已接入面板；角色列表、场景切换及版本冲突检查仍待实施。
...
下一批实施顺序：在角色包与图片外形包基础上补场景组合保存和重载 → 角色列表与配置来源 → 统一语句时间线与独立网页载体；同步补齐首次设置、文件拖放、启动故障呈现和资源路径可迁移性验收。
