...
[0.6.0-alpha.1 二次元外形与轻色工作室](releases/0.6.0-alpha.1.md)改善内置形象、首次体验和桌面布局；当前实现不代表整个数字人路线已经完成。记忆管理和表现投影基础见[0.5 版本记录](releases/0.5.0-alpha.1.md)，早期页面拆分和恢复机制见[0.4 版本记录](releases/0.4.0-alpha.1.md)。
...
近期顺序：先验收 0.6 的人物画面、宽窄窗口操作和旧场景恢复，再回归记忆与真实声音 → VTS/OBS 播放同步与独立网页载体 → 声音与行为包、逐字段配置来源/回退及对应启动恢复。同步补齐首次设置、文件拖放、启动故障呈现和跨机器资源迁移验收；后续版本与下载入口以正式发布记录为准。
