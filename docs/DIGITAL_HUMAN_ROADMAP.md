...
[0.5.0-alpha.1 记忆工作台与连续表达](releases/0.5.0-alpha.1.md)在人物工作室基础上加入可管理记忆与连续表现；当前实现不代表整个数字人路线已经完成。上一版的页面整理、半身人物、发送确认和事件恢复见[0.4 版本记录](releases/0.4.0-alpha.1.md)。
...
- 内置人物支持呼吸、眨眼、视线和表达状态；不依赖 VTS。`PresentationAnimator` 以连续五官参数和有效口型采样实现过渡，停止条件仍立即生效；不会新增随机主动行为。
...
- 内置形象的正式口型已接入原生音频能量与播放进度；离线预览仍用示意动画。人物表情过渡已实现，音素识别与 VTS 播放时间线尚未完成，详见[声音与表达](SPEECH_PRESENTATION.md)。
...
0.2.0-alpha.1 已实现原生语句时间线：入队固定语句身份与情绪，实际播放驱动桌面字幕和原生外形，进度事件避免重复正文，取消与回合切换会抑制旧消息。0.5 以连续五官参数平滑人物表现，静音与停止即时闭嘴；确定性和离屏检查不能替代真实窗口观感、设备延迟、VTS/OBS 与网页同步验收。
...
近期顺序：完成记忆管理、确认笔记与连续表现的整体验收 → VTS/OBS 播放同步与独立网页载体 → 声音与行为包、逐字段配置来源/回退及对应启动恢复；同步补齐首次设置、文件拖放、启动故障呈现和跨机器资源迁移验收。后续版本与下载入口以正式发布记录为准。
