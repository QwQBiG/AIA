...
身份与记忆切换已实现：活动回合拒绝切换，空闲时切换会停止剩余音频、清空短期上下文并选择持久记忆范围。角色包保存、导入与预览应用已接入面板；场景先校验全部资源并准备外形，收到角色应用确认后才切换本地外形。角色列表、完整场景启动恢复及版本冲突检查仍待实施；通信结果不明时保留外形并提示重新确认，尚无跨进程事务恢复。
...
下一批实施顺序：角色列表、配置来源与完整场景启动恢复 → 统一语句时间线与独立网页载体 → 声音与行为包；同步补齐首次设置、文件拖放、启动故障呈现和跨机器资源迁移验收。
