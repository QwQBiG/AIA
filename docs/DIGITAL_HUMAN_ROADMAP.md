...
本文是后续产品与实施计划的主入口，更新于 2026-09-09。下面的目标、模块契约和里程碑属于待实施设计；“当前落地”仅说明当前代码中的基础，不能代替完整交互验收。历史迁移记录继续保留在 [技术基线](CORE_TECHNICAL_BASELINE.md)。

[0.7.0-alpha.1 一号人物与素材工作室](releases/0.7.0-alpha.1.md)接入项目作者的 OC、人物选择与构图、静态素材册和场景兼容；当前实现不代表整个数字人路线已经完成。插画与布局基础见[0.6 版本记录](releases/0.6.0-alpha.1.md)，记忆管理见[0.5 版本记录](releases/0.5.0-alpha.1.md)，早期页面拆分和恢复机制见[0.4 版本记录](releases/0.4.0-alpha.1.md)。
...
载体声明可支持的能力及范围，例如口型、视线、表情混合、骨骼或字幕。原生插画可映射局部状态图片，自定义立绘可切换整张表情图，未来 3D 身体可映射到骨骼动作。能力不足时使用可见的降级说明。
...
- 0.7 新增稳定内置人物目录，默认使用作者的 OC，保留原初始伙伴；OC 的半身/全身与人物 ID 随场景 v2 保存，旧 v1 明确恢复初始伙伴。独立眼口贴层使用自己的校准坐标，六份原画与动作参考在素材册中静态查看，不进入人格或运行时状态。
- 0.6 将默认人物改为程序内嵌的二次元插画，以浅色底圆角卡片呈现。局部表情状态帧与细微呼吸消费原有表现状态，不把插画当成 Live2D 或实时面部变形。自定义透明 PNG/JPEG 包继续可用，舞台点缀不改变人物服装。
- 欢迎页、离线预览、连接设置、聊天和角色页采用暖白淡紫工作室布局。聊天保持固定输入区，人物舞台在宽屏并排、窄屏可折叠；预览和设置的底部操作区保持可达。外形不需要先导入素材，原有身份、收藏、场景与记忆保护继续保留。

- 0.4 桌面拆分聊天、角色与外形、场景、设置页面，预览可直接进入对话；启动会话、导航和人物绘制分别维护。中文输入确认、草稿保留、紧急命令与消息补齐都有独立回归验证。服务实例标识区分重启前后的事件，队列真正接纳消息后才确认发送。

- 0.5 新增[记忆工作台](MEMORY_WORKSPACE.md)：用户可以查看来源、新建确认笔记、更正和单条遗忘。管理请求校验当前角色，修改检查记录版本；成功后清空当前短期上下文与聊天显示，聊天输入草稿保留。结果未知时先刷新核对，不自动重发。
- 记忆区分自动记录、用户笔记和用户更正；确认笔记优先召回，仍受总条数上限限制。长记录明确显示为片段。重复信息在其他记录或角色设定中需要单独处理，概念级遗忘、交互对象分区与关系推导尚未完成。
- `ai-ex-ui-model::PresentationState` 从运行快照生成与渲染器无关的表现状态。
- 桌面支持内置插画、自己的图片与隐藏外形，舞台点缀和减少动态效果偏好通过桌面存储保存。旧内置外形场景继续映射到当前人物。
- `PresentationAnimator` 提供与渲染器无关的状态、呼吸、眨眼和有效口型采样；0.6 默认插画选择局部状态帧，停止条件仍优先生效，不依赖 VTS，也不会新增随机主动行为。
...
- 内置形象的正式口型已接入原生音频能量与播放进度；离线预览仍用示意动画。当前插画采用离散局部帧，音素识别与 VTS 播放时间线尚未完成，详见[声音与表达](SPEECH_PRESENTATION.md)。
...
当前已落地图片外形包、人格角色包及首版场景组合；声音、行为还没有包管理与组合编辑，独立网页/VRM 载体尚未实现。角色间记忆已隔离，并具备记录来源与单条管理；交互对象隔离、关系连续性和完整历史恢复仍待实施。
...
0.2.0-alpha.1 已实现原生语句时间线：入队固定语句身份与情绪，实际播放驱动桌面字幕和原生外形，进度事件避免重复正文，取消与回合切换会抑制旧消息。0.5 加入连续表现参数，0.6 默认插画消费同一状态并选择局部表情帧；静音与停止立即停止说话帧。确定性和离屏检查不能替代真实窗口观感、设备延迟、VTS/OBS 与网页同步验收。

以实际播放进度协调口型、字幕和表情；为每句语音保留身份和情绪，解决模型已经结束、声音仍在播放时表情提前复位的问题。人物表情与视线过渡已有基础，注意力方向、收听反馈和可调动作幅度仍需完善。

先实现语句级同步，再按适配器能力增加词级字幕或音素口型。现有音频能量口型作为基础回退；对不提供情绪语音控制的 TTS 显示真实能力。VTS、原生和网页消费同一时间线，验证慢载体不会拖住打断与其他输出。

### M3：让经历真正留下来

0.5 落地记录级管理与确认笔记召回：[记忆工作台](MEMORY_WORKSPACE.md)支持查看、新建、更正和单条遗忘，并在持久修改成功后重新开始当前上下文。它没有把自动记录视为确认事实，也没有宣称一次删除就处理了所有重复信息。手动流程覆盖重启后查看相同角色笔记；真实模型能否持续遵循偏好仍待用户验收。
...
近期顺序：先验收 0.7 的 OC 眼口表现、半身/全身、素材册与新旧场景恢复，再回归记忆与真实声音 → VTS/OBS 播放同步与独立网页载体 → 声音与行为包、逐字段配置来源/回退及对应启动恢复。同步补齐首次设置、文件拖放、启动故障呈现和跨机器资源迁移验收；后续版本与下载入口以正式发布记录为准。
...
状态、偏好保存和两种外形的绘制测试均在 Rust 中实现。绘制检查覆盖不同表情与窗口宽度、图片纹理与有效网格；离屏图片用于人工检查布局，不能替代完整窗口和音频交互验收。
