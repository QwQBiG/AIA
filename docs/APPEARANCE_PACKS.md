...
正常桌面与离线工作室共用导入功能：选择“图片角色”，展开“导入图片外形包”，拖入文件夹/清单，或粘贴路径后点击“导入 / 重新加载”。可以调整显示大小，或启用“减少动态效果”。
...
同一包的图片最好使用相同画布、人物位置和透明背景，避免切换时跳动。显示时按比例缩放，不拉伸图片；当前尚无图片层叠、嘴部独立图层、骨骼变形或动画序列。
...
正常关闭时保存成功导入包的路径、图片角色选择与显示大小。重新打开后按该路径加载；失败的候选路径不会覆盖成功路径。包仍引用原文件夹，不复制到内部素材库；移动或删除文件后需重新导入，修改图片后点击“重新加载”。离线工作室与正式桌面分别使用各自的窗口偏好存储。
...
验证覆盖清单版本与路径、缺图和大小边界、PNG/JPEG 解码、口型对应的纹理选择、打断/断线回退、后台导入失败与偏好重载。示例生成器已验证新建和拒绝覆盖路径；真实工作室窗口已加载七张图片的示例包。原生文件拖放的完整交互和真实语音长时间联动仍需后续实机验收。
