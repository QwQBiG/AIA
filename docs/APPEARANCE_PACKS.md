...
0.6.0-alpha.1 的默认人物是包含在 `AIex.exe` 内的二次元插画。双击程序，选择“先体验外形”，就能查看人物并切换预览状态；不需要创建文件夹、下载模型或导入素材。

内置插画有浅色画面背景，以圆角卡片呈现，不是透明桌面悬浮人物。八张同画幅素材提供陪伴、眨眼、说话、思考与四种情绪，运行时用固定主体与局部表情状态帧叠加表现变化，辅以细微呼吸；当前没有 Live2D、骨骼或实时五官变形。舞台配色仅调整装饰，不重新绘制人物和服装。
