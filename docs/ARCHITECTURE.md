...
本文描述 0.5.0-alpha.1 的记忆工作台与连续表达架构。未来设计见[演进路线](DIGITAL_HUMAN_ROADMAP.md)，验收入口见[整体验收流程](MANUAL_TEST_PLAN.md)。
...
`PresentationAnimator` 位于 `ai-ex-ui-model`，每个外形实例保存自己的瞬时过渡状态。它把运行状态投影为连续眉形、眼形、嘴角、腮红、视线和口型参数；绘制模块只负责映射几何。平滑不改变运行时事实：静音、失同步、断线、打断和减少动态立即闭嘴，不延长旧音节。该历史不落盘，不生成主动行为，也不改变图片包的离散表情资源。
