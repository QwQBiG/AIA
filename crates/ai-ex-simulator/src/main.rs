...
use ai_ex_event_bus::{
    EventBus, EventPolicy, PublishOutcome, load_jsonl, project_memory, replay_delay,
};
...
async fn run() -> Result<(), AppError> {
    ...
    let mut report = options
        ...
        .map(|path| {
            File::create(path).map(BufWriter::new).map_err(|error| {
                AppError::unavailable(format!("cannot create simulation report: {error}"))
            })
        })
        .transpose()?;
    ...
    for event in events {
        ...
        let (event_type, reaction_prompt, memory_projections) =
        ...
        if let Some(writer) = report.as_mut() {
            ...
            serde_json::to_writer(&mut *writer, &record).map_err(|error| {
                AppError::protocol(format!("cannot encode simulation report: {error}"))
            })?;
            writer.write_all(b"\n").map_err(|error| {
                AppError::unavailable(format!("cannot write simulation report: {error}"))
            })?;
        }
    ...
    if let Some(writer) = report.as_mut() {
        writer.flush().map_err(|error| {
            AppError::unavailable(format!("cannot flush simulation report: {error}"))
        })?;
    }
    println!(
