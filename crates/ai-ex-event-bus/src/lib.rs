...
pub fn project_memory(event: &LiveEventEnvelope) -> Vec<MemoryProjection> {
    let projection =
        |kind: MemoryKind, user_text: String, assistant_text: String| MemoryProjection {
            kind,
...
impl LiveEvent {
    ...
    pub fn summary(&self) -> String {
        let summary = match self {
            ...
            Self::Moderation { action, reason, .. } => {
                format!("平台管理动作：{action}（{reason}）")
            }
    ...
    pub fn reaction_prompt(&self) -> Option<String> {
        ...
        match self {
            ...
            Self::Follow { display_name, .. } => Some(format!(
...
impl EventBus {
    ...
    pub fn publish(&mut self, event: LiveEventEnvelope) -> PublishOutcome {
        ...
        if let Some(key) = event.payload.dedupe_key() {
            if self.seen.get(&key).is_some_and(|previous| {
                now.saturating_sub(*previous) < self.policy.per_user_cooldown_ms
            }) {
        ...
        if let Some(user_id) = event.payload.user_key() {
            if self.last_user.get(user_id).is_some_and(|previous| {
                now.saturating_sub(*previous) < self.policy.per_user_cooldown_ms
            }) {
        ...
        if event.payload.has_global_cooldown()
            && self.last_global.is_some_and(|previous| {
                now.saturating_sub(previous) < self.policy.global_cooldown_ms
            })
        {
...
impl JsonlRecorder {
    ...
    pub fn record(&mut self, event: &LiveEventEnvelope) -> Result<(), AppError> {
        serde_json::to_writer(&mut self.writer, event).map_err(|error| {
            AppError::protocol(format!("cannot encode event recording: {error}"))
        })?;
        self.writer.write_all(b"\n").map_err(|error| {
            AppError::unavailable(format!("cannot write event recording: {error}"))
        })?;
        Ok(())
    }
    ...
    pub fn flush(&mut self) -> Result<(), AppError> {
        self.writer.flush().map_err(|error| {
            AppError::unavailable(format!("cannot flush event recording: {error}"))
        })
    }
}
...
pub fn load_jsonl(path: impl AsRef<Path>) -> Result<Vec<LiveEventEnvelope>, AppError> {
    let file = File::open(path.as_ref())
        .map_err(|error| AppError::unavailable(format!("cannot open event recording: {error}")))?;
    let reader = BufReader::new(file);
    let mut events = Vec::new();
    for (line_number, line) in reader.lines().enumerate() {
        let line = line.map_err(|error| {
            AppError::unavailable(format!(
                "cannot read event line {}: {error}",
                line_number + 1
            ))
        })?;
...
mod tests {
    ...
    fn bus_deduplicates_and_applies_cooldowns() {
        ...
        assert_eq!(
            bus.publish(chat(1_050, "m-2")),
            PublishOutcome::UserCooldown
        );
    ...
    fn projects_live_events_to_memory_kinds() {
        ...
        assert!(
            projections
                .iter()
                .any(|item| item.kind == MemoryKind::Viewer)
        );
        assert!(
            projections
                .iter()
                .any(|item| item.kind == MemoryKind::LiveEvent)
        );
        assert!(
            projections
                .iter()
                .all(|item| item.event_id == event.event_id)
        );
