...
pub(super) fn encode(entries: &[LibraryEntry]) -> Result<String, AppError> {
    if entries.len() > MAX_ENTRIES {
        return Err(failure("the library supports at most 32 saved versions"));
    }
    let mut values = Vec::new();
    for (index, entry) in entries.iter().enumerate() {
        if entry.source.chars().count() > 2048
            || entries[..index].iter().any(|other| {
                other.id == entry.id || same_version(&other.character, &entry.character)
            })
        {
            return Err(failure(
                "duplicate identity/revision or invalid source description",
            ));
        }
    ...
    if text.len() > MAX_BYTES {
        return Err(failure("the library exceeds 2 MiB"));
    }
    Ok(text)
}
...
fn decode(text: &str) -> Result<Vec<LibraryEntry>, AppError> {
    ...
    let values = value["entries"]
        .as_array()
        .ok_or_else(|| failure("missing library entries"))?;
    if values.len() > MAX_ENTRIES {
        return Err(failure("too many library entries"));
    }
    let mut entries = Vec::new();
    for value in values {
        if value.as_object().is_none_or(|object| object.len() != 3) {
            return Err(failure("invalid library entry fields"));
        }
        let id = value["id"]
            .as_str()
            .ok_or_else(|| failure("missing entry ID"))?;
        let character = value["character"]
            .as_str()
            .ok_or_else(|| failure("missing character snapshot"))?;
        let source = value["source"]
            .as_str()
            .ok_or_else(|| failure("missing source description"))?;
        entries.push(LibraryEntry {
            id: Uuid::parse_str(id).map_err(failure)?,
            character: CharacterManifest::parse(character)?,
...
impl CharacterLibrary {
    ...
    pub fn save(&mut self, storage: &mut dyn eframe::Storage) -> Result<(), AppError> {
        ...
        if let Some(original) = self.original.take() {
            storage.set_string(
                &format!("character.library.recovery.{}", Uuid::new_v4()),
                original,
            );
        }
...
fn examples() -> Result<Vec<LibraryEntry>, AppError> {
    [
        include_str!("../../../config/characters/companion.toml"),
        include_str!("../../../config/characters/host.toml"),
    ]
    .into_iter()
    .enumerate()
    .map(|(index, text)| {
        Ok(LibraryEntry {
            ...
            character: CharacterManifest::parse(text)?,
            source: "AIex 内置示例".to_owned(),
        })
    })
    .collect()
}
