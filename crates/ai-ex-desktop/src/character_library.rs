...
impl CharacterLibrary {
    pub fn remember(
        ...
        if let Some(existing) = self
            .entries
            .iter()
            .find(|entry| same_version(&entry.character, &character))
        {
            if existing.character == character {
                return Ok(());
            }
            return Err(failure(
                "this identity and revision already contain different settings; increase the revision before saving",
            ));
        }
    ...
    pub fn remove(&mut self, index: usize) -> Result<(), AppError> {
        if index >= self.entries.len() {
            return Err(failure("selection is no longer available"));
        }
        let mut entries = self.entries.clone();
    ...
    pub fn undo_remove(&mut self) -> Result<(), AppError> {
        let Some((index, entry)) = self.removed.clone() else {
            return Ok(());
        };
        let mut entries = self.entries.clone();
...
fn same_version(first: &CharacterManifest, second: &CharacterManifest) -> bool {
    first.persona.profile_id == second.persona.profile_id
        && first.persona.revision == second.persona.revision
}
