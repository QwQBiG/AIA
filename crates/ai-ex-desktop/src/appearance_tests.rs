...
fn appearance_preferences_round_trip_and_invalid_color_falls_back()
{
    ...
    let panel = AppearancePanel { kind: AppearanceKind::Orb, accent: [12, 34, 56], reduced_motion: true, ..Default::default() };
    ...
    storage.0.insert("appearance.kind".to_owned(), "images".to_owned());
    storage.0.insert("appearance.image_source".to_owned(), "my-character/appearance.toml".to_owned());
    storage.0.insert("appearance.image_scale".to_owned(), "0.8".to_owned());
    let restored = AppearancePanel::load(Some(&storage));
    ...
    storage.0.insert("appearance.accent".to_owned(), "999,2".to_owned());
    assert_eq!(AppearancePanel::load(Some(&storage)).accent, AppearancePanel::default().accent);
}
...
fn both_renderers_tessellate_all_expressions_at_compact_and_large_sizes()
{
    ...
    {
        for emotion in [Emotion::Neutral, Emotion::Happy, Emotion::Sad, Emotion::Angry, Emotion::Surprised]
        {
            for width in [320.0, 760.0]
            {
                let context = egui::Context::default();
                let mut panel = AppearancePanel { kind, ..Default::default() };
                let state = PresentationState {
                    connected: true, synchronized: true,
                    activity: ConversationState::Speaking, emotion,
                    mouth_level: None,
                };
                let output = context.run_ui(egui::RawInput {
                    screen_rect: Some(egui::Rect::from_min_size(egui::Pos2::ZERO, egui::vec2(width, 480.0))),
                    time: Some(1.0),
                    ..Default::default()
                ...
                for primitive in primitives
                {
                    if let egui::epaint::Primitive::Mesh(mesh) = primitive.primitive
                    {
                        ...
                        assert!(mesh.vertices.iter().all(|vertex| vertex.pos.x.is_finite() && vertex.pos.y.is_finite()));
                    }
                }
            }
        }
...
fn image_pack_preference_restores_loaded_body_after_restart()
{
    ...
    image::RgbaImage::new(2, 2).save(root.join("face.png")).unwrap();
    std::fs::write(root.join("appearance.toml"), "schema_version = 1\nid = 'restore.test'\nname = 'Restored friend'\n[images]\ndefault = 'face.png'\n").unwrap();
    let context = egui::Context::default();
    let mut panel = AppearancePanel::default();
    panel.use_images(&context, crate::image_appearance::DecodedAppearance::load(&root).unwrap());
    let mut storage = Storage::default();
    panel.save(&mut storage);
    ...
    {
        assert!(!restored.images.poll(&context), "restoring should retain the saved body selection");
        assert!(restored.images.error.is_none());
        assert!(std::time::Instant::now() < deadline);
        std::thread::sleep(std::time::Duration::from_millis(10));
    }
    assert_eq!(restored.images.current.as_ref().unwrap().manifest.id, "restore.test");
