...
impl AppearanceImport
{
    ...
    {
        let source = storage.and_then(|storage| storage.get_string("appearance.image_source")).unwrap_or_default();
        Self { restore: !source.is_empty(), input: source.clone(), source, ..Default::default() }
    }
    ...
    pub fn begin(&mut self, context: &egui::Context, path: PathBuf)
    {
        ...
        match std::thread::Builder::new().name("ai-ex-appearance-import".to_owned()).spawn(move ||
        {
    ...
    pub fn poll(&mut self, context: &egui::Context) -> bool
    {
        ...
        let result = match pending.try_recv()
        {
            ...
            Err(TryRecvError::Disconnected) => Err(AppError::unavailable("appearance import worker stopped")),
        };
        self.pending = None;
        match result
        {
    ...
    pub fn controls(&mut self, ui: &mut egui::Ui)
    {
        ...
        ui.collapsing("导入图片外形包", |ui|
        {
            ...
            ui.add(egui::TextEdit::singleline(&mut self.input).desired_width(f32::INFINITY).hint_text("外形清单路径"));
            if ui.add_enabled(!self.is_loading() && !self.input.trim().is_empty(), egui::Button::new("导入 / 重新加载")).clicked()
            {
        ...
        {
            ui.weak(format!("外形包：{} · {} 张图片", pack.manifest.name, pack.manifest.images.len()))
                .on_hover_text(pack.source.display().to_string());
        }
        if let Some(error) = &self.error
        {
            ui.colored_label(egui::Color32::LIGHT_RED, "外形导入失败，已保留原外形（悬停查看原因）").on_hover_text(error);
        }
