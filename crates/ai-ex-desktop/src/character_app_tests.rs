...
fn persona_polling_never_applies_a_draft_or_acknowledges_a_pending_command() {
    ...
    events
        ...
        .unwrap();
    events
        .send(WorkerEvent::Failure("unrelated poll failure".to_owned()))
        .unwrap();
    app.drain_events();
    assert!(app.persona_apply_pending);
    assert_eq!(app.persona, draft);
    events
        .send(WorkerEvent::PersonaApplied(draft.clone()))
        .unwrap();
    app.drain_events();
    ...
    app.persona_apply_pending = true;
    events
        .send(WorkerEvent::PersonaApplyFailed("rejected".to_owned()))
        .unwrap();
    app.drain_events();
    assert!(!app.persona_apply_pending);
