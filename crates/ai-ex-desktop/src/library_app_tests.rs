...
fn library_selection_waits_for_service_confirmation_and_tracks_its_source()
{
    ...
    let output = context.run_ui(egui::RawInput { screen_rect: Some(egui::Rect::from_min_size(egui::Pos2::ZERO, egui::vec2(720.0, 520.0))), ..Default::default() }, |ui| app.show_character_library(ui));
    ...
    assert_ne!(app.character_draft(), *app.character_files.baseline.as_ref().unwrap());
    app.state.connection = ConnectionState::Connected;
    app.apply_pending_persona(app.persona.clone());
    let WorkerCommand::SetPersona(profile) = sent.try_recv().unwrap() else { panic!("expected selected persona"); };
    ...
    assert_eq!(app.character_draft(), *app.character_files.baseline.as_ref().unwrap());
...
fn independent_library_copy_has_a_new_identity_and_preserves_original_attribution()
{
    ...
    assert_ne!(first.character.persona.profile_id, original.persona.profile_id);
    assert_ne!(first.character.persona.profile_id, second.character.persona.profile_id);
