...
impl Fixture {
    fn new() -> Self {
        ...
        for (name, rgba) in [
            ("default.png", [20, 80, 160, 255]),
            ("speaking.png", [120, 200, 20, 255]),
        ] {
...
fn decodes_png_and_jpeg_and_rejects_corrupt_or_oversized_images() {
    ...
    image::RgbImage::from_pixel(4, 4, image::Rgb([100, 90, 80]))
        .save(fixture.0.join("photo.jpg"))
        .unwrap();
    let manifest = std::fs::read_to_string(fixture.0.join("appearance.toml")).unwrap();
    std::fs::write(
        fixture.0.join("appearance.toml"),
        manifest.replace("default.png", "photo.jpg"),
    )
    .unwrap();
    assert_eq!(
        DecodedAppearance::load(&fixture.0).unwrap().images["default"].size,
        [4, 4]
    );
    std::fs::write(fixture.0.join("appearance.toml"), manifest).unwrap();
    image::RgbaImage::new(2049, 1)
        .save(fixture.0.join("default.png"))
        .unwrap();
    assert!(
        DecodedAppearance::load(&fixture.0)
            .err()
            .unwrap()
            .to_string()
            .contains("2048")
...
fn renderer_changes_texture_with_audio_and_stops_on_disconnect_or_interrupt() {
    ...
    for (connected, activity, level, expected) in [
        ...
        let state = PresentationState {
            ...
            mouth_level: Some(level),
        };
        let output = context.run_ui(egui::RawInput::default(), |ui| {
            pack.draw(
                ui.painter(),
                Rect::from_min_size(egui::pos2(0.0, 0.0), egui::vec2(200.0, 180.0)),
                state,
...
fn failed_background_import_preserves_current_appearance() {
    ...
    while importer.error.is_none() {
        ...
        assert!(
            ...
            "background import should finish"
        );
    ...
    while importer.is_loading() {
        assert!(
            !importer.poll(&context),
            "finishing import must not override a newer body selection"
        );
