...
impl ResumeLaunch
{
    ...
    {
        let path = config.canonicalize().map_err(|error| AppError::configuration(format!("cannot identify desktop configuration: {error}")))?;
        let path = path.to_str().ok_or_else(|| AppError::configuration("configuration path must be UTF-8"))?;
        Ok(Self { scope: format!("scene.startup.v1.{}:{path}:{address}", path.len()), automatic })
    }
}
...
impl ResumeSnapshot
{
    pub fn validate(&self) -> Result<(), AppError>
    {
        ...
        {
            return Err(AppError::configuration("startup scene image source is incomplete"));
        }
        if let Some(path) = &self.source
            && (!path.is_absolute() || path.to_str().is_none_or(|path| path.len() > 32_768))
        {
            return Err(AppError::configuration("startup image source must be an absolute UTF-8 path"));
        }
        Ok(())
    }
}
...
impl SceneResume
{
    ...
    {
        let mut result = Self { scope: launch.scope, automatic: launch.automatic, available: storage.is_some(), ..Default::default() };
        if let Some(storage) = storage
            && let Some(text) = storage.get_string(&format!("{}.manifest", result.scope))
        {
            let source = storage.get_string(&format!("{}.source", result.scope)).filter(|value| !value.is_empty()).map(PathBuf::from);
            ...
            match parsed
            {
                Ok(snapshot) => { result.snapshot = Some(snapshot); result.phase = ResumePhase::Queued; }
                Err(error) => result.feedback = Some(format!("启动组合不可用，保留原设置：{error}")),
            }
        }
        result
    }
    ...
    pub fn save(&mut self, storage: &mut dyn eframe::Storage) -> Result<(), AppError>
    {
        ...
        if let Some(snapshot) = &self.snapshot
        {
            ...
            storage.set_string(&source_key, snapshot.source.as_ref().and_then(|path| path.to_str()).unwrap_or_default().to_owned());
