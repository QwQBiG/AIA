...
fn snapshot() -> ResumeSnapshot {
    ...
    ResumeSnapshot {
        scene: ai_ex_config::scene::SceneBundle::load(&path)
            .unwrap()
            .manifest,
        source: None,
    }
...
fn startup_snapshot_is_scoped_validated_and_preserved_until_explicitly_replaced() {
    ...
    let mut resume = SceneResume::load(
        Some(&storage),
        ResumeLaunch {
            scope: "one".to_owned(),
            automatic: true,
    ...
    storage
        .0
        .insert("one.manifest".to_owned(), "broken TOML".to_owned());
    let mut failed = SceneResume::load(
        Some(&storage),
        ResumeLaunch {
            scope: "one".to_owned(),
            automatic: true,
...
fn startup_scope_uses_the_resolved_configuration_and_service_address() {
    ...
    assert_ne!(
        first.scope,
        ResumeLaunch::new(&root.join("host.toml"), "127.0.0.1:8000", true)
            .unwrap()
            .scope
    );
    assert_ne!(
        first.scope,
        ResumeLaunch::new(&root.join("companion.toml"), "127.0.0.1:9000", true)
            .unwrap()
            .scope
    );
