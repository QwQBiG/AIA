...
pub fn save(path: &Path, document: &str, original: Option<&str>) -> Result<(), AppError> {
    ...
    if current.as_deref() != original {
        return Err(AppError::configuration(
            "configuration changed while setup was open; reopen setup before saving",
        ));
    }
    let path = std::path::absolute(path).map_err(failure)?;
    let parent = path
        .parent()
        .ok_or_else(|| AppError::configuration("configuration requires a directory"))?;
        ...
        .open(token_path)
    {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            ...
            if token.trim().len() < 32 {
                return Err(AppError::configuration(
                    "existing control token is invalid; it was not replaced",
                ));
            }
        }
        Err(error) => return Err(failure(error)),
