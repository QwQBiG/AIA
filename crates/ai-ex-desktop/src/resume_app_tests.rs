...
impl Rig
{
    fn new(automatic: bool) -> Self
    {
        ...
        let scene = ai_ex_config::scene::SceneBundle::load(&path).unwrap().manifest;
        let launch = || ResumeLaunch { scope: "test".to_owned(), automatic };
        let mut resume = SceneResume::load(Some(&storage), launch());
        resume.snapshot = Some(ResumeSnapshot { scene, source: None });
        resume.dirty = true;
        resume.save(&mut storage).unwrap();
        let (events, receiver) = std::sync::mpsc::channel();
        let (sender, commands) = tokio::sync::mpsc::unbounded_channel();
        let mut app = DesktopApp::with_storage(WorkerHandle { commands: sender, events: receiver }, false, None);
        app.resume = SceneResume::load(Some(&storage), launch());
        Self { app, events, commands, storage }
    }
    ...
    fn finish_loading(&mut self, context: &egui::Context)
    {
        ...
        loop
        {
            ...
            if !matches!(self.app.resume.phase, ResumePhase::Queued | ResumePhase::Loading) { break; }
    ...
    fn connect(&mut self)
    {
        ...
        self.events.send(WorkerEvent::Persona(PersonaSnapshot::default())).unwrap();
...
fn startup_restore_waits_for_connection_profile_sync_and_its_command_ack()
{
    ...
    rig.events.send(WorkerEvent::Persona(PersonaSnapshot::default())).unwrap();
    rig.app.drain_events();
    rig.events.send(WorkerEvent::Connection(true)).unwrap();
    rig.app.drain_events();
    rig.app.poll_scene(&context);
    assert!(rig.commands.try_recv().is_err(), "an offline profile must not authorize automatic restore");
    rig.app.input = "Hello after restore".to_owned();
    rig.app.submit();
    assert!(rig.commands.try_recv().is_err(), "new input must wait for the selected identity");
    assert_eq!(rig.app.input, "Hello after restore");
    rig.connect();
    rig.app.poll_scene(&context);
    let WorkerCommand::SetPersona(profile) = rig.commands.try_recv().unwrap() else { panic!("expected restore"); };
    ...
    rig.events.send(WorkerEvent::PersonaApplied(profile.clone())).unwrap();
    rig.app.drain_events();
    ...
    rig.app.submit();
    assert!(matches!(rig.commands.try_recv(), Ok(WorkerCommand::Submit(text)) if text == "Hello after restore"));
    rig.app.appearance.kind = AppearanceKind::Hidden;
    rig.app.save_resume(&mut rig.storage);
    let restored = SceneResume::load(Some(&rig.storage), ResumeLaunch { scope: "test".to_owned(), automatic: true });
    assert_eq!(restored.snapshot.unwrap().scene.appearance.body, ai_ex_config::scene::SceneBody::Orb);
}
...
fn startup_restore_on_existing_service_needs_confirmation_and_never_retries_a_rejection()
{
    ...
    rig.finish_loading(&context);
    rig.app.apply_pending_persona(rig.app.pending_persona.clone().unwrap());
    assert!(matches!(rig.commands.try_recv(), Ok(WorkerCommand::SetPersona(_))));
    rig.events.send(WorkerEvent::PersonaApplyFailed("active turn".to_owned())).unwrap();
    rig.app.drain_events();
    for _ in 0..5 { rig.app.poll_scene(&context); }
    assert_eq!(rig.app.active_persona.profile_id, "default");
    assert_eq!(rig.app.appearance.kind, AppearanceKind::Companion);
    assert!(!rig.app.scene_busy());
    assert!(rig.commands.try_recv().is_err());
    rig.app.save_resume(&mut rig.storage);
    assert!(SceneResume::load(Some(&rig.storage), ResumeLaunch { scope: "test".to_owned(), automatic: true }).snapshot.is_some());
}
...
fn broken_startup_images_do_not_change_identity_or_erase_the_saved_choice()
{
    ...
    snapshot.source = Some(std::env::temp_dir().join(format!("missing-aiex-image-{}.toml", uuid::Uuid::new_v4())));
    ...
    for _ in 0..5 { rig.app.poll_scene(&context); }
    assert!(rig.app.resume.phase == ResumePhase::Idle);
    assert!(rig.app.scene_files.feedback.as_ref().unwrap().contains("失败"));
    assert_eq!(rig.app.active_persona.profile_id, "default");
    assert_eq!(rig.app.appearance.kind, AppearanceKind::Companion);
    assert!(rig.commands.try_recv().is_err());
    assert!(rig.app.resume.snapshot.is_some());
    ...
    assert_eq!(rig.app.resume.snapshot.as_ref().unwrap().scene.character.persona, rig.app.active_persona);
    assert_ne!(rig.app.resume.snapshot.as_ref().unwrap().scene.character.persona.name, "Unapplied draft");
    rig.app.save_resume(&mut rig.storage);
    let restored = SceneResume::load(Some(&rig.storage), ResumeLaunch { scope: "test".to_owned(), automatic: true });
    assert_eq!(restored.snapshot.unwrap().scene.appearance.body, ai_ex_config::scene::SceneBody::Companion);
}
...
fn startup_image_scene_round_trips_its_source_and_commits_only_after_confirmation()
{
    ...
    image::RgbaImage::from_pixel(8, 8, image::Rgba([90, 170, 240, 255])).save(root.join("default.png")).unwrap();
    let mut rig = Rig::new(true);
    let snapshot = rig.app.resume.snapshot.as_mut().unwrap();
    snapshot.scene.appearance.body = ai_ex_config::scene::SceneBody::Images;
    snapshot.scene.appearance.package = Some("appearance/appearance.toml".to_owned());
    ...
    rig.app.resume.dirty = true;
    rig.app.save_resume(&mut rig.storage);
    rig.app.resume = SceneResume::load(Some(&rig.storage), ResumeLaunch { scope: "test".to_owned(), automatic: true });
    let context = egui::Context::default();
    rig.connect();
    rig.finish_loading(&context);
    let WorkerCommand::SetPersona(profile) = rig.commands.try_recv().unwrap() else { panic!("expected restore"); };
    assert_eq!(rig.app.appearance.kind, AppearanceKind::Companion);
    rig.events.send(WorkerEvent::PersonaApplied(profile)).unwrap();
    rig.app.drain_events();
    ...
    {
        rig.app.appearance.show(ui, state, &rig.app.active_persona.name, 150.0);
    });
    let primitives = context.tessellate(output.shapes, output.pixels_per_point);
    assert!(!primitives.is_empty());
    assert!(primitives.iter().all(|primitive| match &primitive.primitive
    {
