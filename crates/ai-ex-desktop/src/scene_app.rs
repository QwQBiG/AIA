...
impl DesktopApp {
    pub(super) fn poll_scene(&mut self, context: &egui::Context) {
        ...
        if let Some(ready) = self.scene_files.poll(context) {
            ...
            if restoring {
                self.resume.phase = if self.pending_scene.is_some() {
                    crate::scene_resume::ResumePhase::Prepared
                } else {
                    crate::scene_resume::ResumePhase::Idle
                };
            }
    ...
    pub(super) fn scene_busy(&self) -> bool {
        self.scene_files.is_loading()
            || self.pending_scene.is_some()
            || self.applying_scene.is_some()
    }

    pub(super) fn prepare_scene(&mut self, context: &egui::Context, ready: ReadyScene) {
        let prepared = ready.manifest.validate().and_then(|()| {
            AppearancePanel::from_scene(context, &ready.manifest.appearance, ready.appearance)
        });
        match prepared {
            Ok(appearance) => {
                self.pending_persona = Some(ready.manifest.character.persona.clone());
                self.scene_files.feedback = Some(format!(
                    "已准备场景“{}”，确认后切换角色与外形。",
                    ready.manifest.name
                ));
            ...
            Err(error) => {
                self.scene_files.feedback = Some(format!("无法准备场景，当前组合保留：{error}"))
            }
        }
    }
    ...
    pub(super) fn finish_scene(&mut self, profile: &PersonaSnapshot) {
        if let Some(scene) = self.applying_scene.take() {
            ...
            self.active_source = if self.character_files.draft_source.is_empty() {
                "手动角色设置".to_owned()
            } else {
                self.character_files.draft_source.clone()
            };
            if self
                .character_files
                .baseline
                .as_ref()
                .is_some_and(|baseline| baseline != &self.active_character)
    ...
    pub(super) fn show_scene_confirmation(&self, ui: &mut egui::Ui) {
        if let Some(scene) = &self.pending_scene {
            if self.resume.phase == crate::scene_resume::ResumePhase::Prepared {
                ui.label(if self.resume.automatic {
                    "正在等待服务连接与角色同步，以恢复启动组合；可以取消。"
                } else {
                    "这是已记住的启动组合，确认后恢复到当前服务。"
                });
            }
            ...
            ui.colored_label(
                ...
                format!(
                    "外形：{body} · 图片大小 {:.0}% · 减少动态：{}",
                    preset.scale * 100.0,
                    if preset.reduced_motion { "是" } else { "否" }
                ),
            );
