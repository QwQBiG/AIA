...
impl DesktopApp
{
    ...
    pub(super) fn show_character_library(&mut self, ui: &mut egui::Ui)
    {
        ...
        ui.label(format!("当前角色：{} · v{}", self.active_persona.name, self.active_persona.revision));
        ui.weak(format!("当前来源：{}", if self.active_source.is_empty() { "等待服务同步" } else { &self.active_source }));
        let modified = self.character_files.baseline.as_ref().is_some_and(|baseline| baseline != &draft);
        ui.weak(format!("草稿来源：{}{}", if self.character_files.draft_source.is_empty() { "手动角色设置" } else { &self.character_files.draft_source }, if modified { "（已修改）" } else { "" }));
        ui.add_enabled_ui(!self.persona_apply_pending && !self.confirm_persona && !self.character_files.is_loading(), |ui|
        {
            if let Some(selected) = self.character_library.show(ui, &self.active_persona, &draft, &self.character_files.draft_source)
            {
