...
impl CharacterFiles {
            ...
            ui.add_enabled_ui(!self.is_loading(), |ui| {
                ui.add(
                    egui::TextEdit::singleline(&mut self.path)
                        ...
                        .hint_text("character.toml 文件或已有文件夹路径"),
                );
                ui.horizontal(|ui| {
                    ui.label("作者");
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add_enabled(
                            !self.is_loading() && !self.path.trim().is_empty(),
                            egui::Button::new("导出当前草稿"),
                        )
                        .clicked()
                    {
    ...
    pub fn begin(&mut self, context: &egui::Context, action: FileAction) {
        ...
        match std::thread::Builder::new()
            .name("ai-ex-character-files".to_owned())
            .spawn(move || {
    ...
    pub fn poll(&mut self, context: &egui::Context) -> Option<CharacterManifest> {
        ...
        let result = match pending.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => {
                Err(AppError::unavailable("character file worker stopped"))
            }
        };
        self.pending = None;
        match result {
