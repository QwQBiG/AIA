...
fn background_character_files_preserve_metadata_on_failure_and_refuse_overwrite() {
    let directory =
        std::env::temp_dir().join(format!("aiex-character-files-{}", uuid::Uuid::new_v4()));
    ...
    files.begin(
        &context,
        FileAction::Export(path.clone(), Box::new(manifest.clone())),
    );
    ...
    files.begin(
        &context,
        FileAction::Export(path.clone(), Box::new(manifest)),
    );
    assert!(finish(&mut files, &context).is_none());
