...
impl AppearancePanel
{
    ...
    pub fn scene_snapshot(&self) -> Result<(SceneAppearance, Option<PathBuf>), AppError>
    {
        if self.is_loading()
        {
            return Err(AppError::configuration("wait for the appearance import to finish"));
        }
        ...
        {
            Some(self.images.current.as_ref().ok_or_else(|| AppError::configuration("import an image package before exporting this scene"))?.source.clone())
        }
        else { None };
        Ok((SceneAppearance {
            body, accent: self.accent, reduced_motion: self.reduced_motion, scale: self.image_scale,
            package: source.as_ref().map(|_| "appearance/appearance.toml".to_owned()),
        }, source))
    }

    pub fn from_scene(context: &egui::Context, preset: &SceneAppearance, decoded: Option<DecodedAppearance>) -> Result<Self, AppError>
    {
        ...
        let mut panel = Self {
            kind, accent: preset.accent, reduced_motion: preset.reduced_motion,
            image_scale: preset.scale, ..Default::default()
        };
        if let Some(decoded) = decoded { panel.use_images(context, decoded); }
        Ok(panel)
    }
}
