...
fn ready() -> ReadyScene {
    let mut character = CharacterManifest::from_persona(PersonaSnapshot {
        profile_id: "friend".to_owned(),
...
fn scene_waits_for_its_ack_before_committing_identity_and_appearance() {
    ...
    assert_eq!(
        app.appearance.scene_snapshot().unwrap().0,
        ready().manifest.appearance
    );
...
fn background_scene_import_reaches_confirmation_without_changing_the_editor() {
    ...
    app.persona_dirty = true;
    app.scene_files
        .begin(&context, crate::scene_files::SceneAction::Import(path));
    ...
    assert!(app.confirm_persona);
    assert_eq!(
        app.pending_persona.as_ref().unwrap().profile_id,
        "aiex.companion"
    );
    assert_eq!(app.active_persona.profile_id, "default");
