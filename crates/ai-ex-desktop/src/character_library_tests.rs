...
fn library_preserves_versions_round_trips_snapshots_and_undoes_removal()
{
    ...
    library.remember(original.clone(), "moved/file.toml".to_owned()).unwrap();
    assert_eq!(library.entries.len(), 2);
    let mut edited = original.clone();
    edited.persona.name = "Edited".to_owned();
    assert!(library.remember(edited.clone(), "draft".to_owned()).is_err());
    assert_eq!(library.entries[0].character, original);
    edited.persona.revision += 1;
    library.remember(edited.clone(), "missing/file.toml".to_owned()).unwrap();
...
{
    let mut storage = Storage::default();
    storage.0.insert(STORAGE_KEY.to_owned(), "broken JSON".to_owned());
    let mut library = CharacterLibrary::load(Some(&storage));
    assert!(library.feedback.is_some());
    library.save(&mut storage).unwrap();
    assert_eq!(storage.0[STORAGE_KEY], "broken JSON");
    library.remember(CharacterManifest::from_persona(Default::default()), "draft".to_owned()).unwrap();
    library.save(&mut storage).unwrap();
    assert!(storage.0.iter().any(|(key, value)| key.starts_with("character.library.recovery.") && value == "broken JSON"));
...
fn library_rejects_capacity_overflow_duplicate_records_and_oversized_sources()
{
    ...
    assert!(library.remember(character.clone(), "draft".to_owned()).is_err());
    assert_eq!(library.entries.len(), 32);
    library.remove(0).unwrap();
    assert!(library.remember(character.clone(), "x".repeat(2049)).is_err());
    library.remember(character, "draft".to_owned()).unwrap();
    ...
    storage.0.insert(STORAGE_KEY.to_owned(), " ".repeat(MAX_BYTES + 1));
    assert!(CharacterLibrary::load(Some(&storage)).feedback.is_some());
}
