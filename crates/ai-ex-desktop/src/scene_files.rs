    ...
    match action {
        SceneAction::Restore(snapshot) => {
            ...
            let appearance = snapshot
                .source
                .as_ref()
                .map(|path| DecodedAppearance::load(path))
                .transpose()?;
            Ok(SceneResult::Loaded(Box::new(ReadyScene {
                manifest: snapshot.scene,
                appearance,
            })))
        }
        SceneAction::Import(path) => {
            let bundle = SceneBundle::load(&path)?;
            let appearance = bundle
                .appearance
                .map(DecodedAppearance::from_loaded)
                .transpose()?;
            Ok(SceneResult::Loaded(Box::new(ReadyScene {
        ...
        SceneAction::Export(path, manifest, source) => {
            let appearance = source
                .map(|path| LoadedAppearance::load(&path))
                .transpose()?;
            if let Some(loaded) = &appearance {
                DecodedAppearance::from_loaded(loaded.clone())?;
            }
...
impl SceneFiles {
    ...
    pub fn begin(&mut self, context: &egui::Context, action: SceneAction) {
        ...
        match std::thread::Builder::new()
            .name("ai-ex-scene-files".to_owned())
            .spawn(move || {
    ...
    pub fn poll(&mut self, context: &egui::Context) -> Option<ReadyScene> {
        ...
        let result = match pending.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => {
                Err(AppError::unavailable("scene file worker stopped"))
            }
        };
        self.pending = None;
        match result {
