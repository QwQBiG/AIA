...
fn scene_export_copies_validated_image_bytes_and_rejects_corrupt_images()
{
    ...
    image::RgbaImage::from_pixel(8, 8, image::Rgba([80, 120, 180, 255])).save(root.join("default.png")).unwrap();
    let manifest = SceneManifest {
        schema_version: 1, id: "images".to_owned(), name: "图片角色".to_owned(),
        character: CharacterManifest::from_persona(Default::default()),
        appearance: SceneAppearance { body: SceneBody::Images, accent: [30, 60, 90], reduced_motion: true, scale: 0.7, package: Some("appearance/appearance.toml".to_owned()) },
    };
    let target = root.join("exported");
    assert!(matches!(execute(SceneAction::Export(target.clone(), Box::new(manifest.clone()), Some(source.clone()))).unwrap(), SceneResult::Saved(_)));
    std::fs::remove_file(root.join("default.png")).unwrap();
    std::fs::remove_file(&source).unwrap();
    let SceneResult::Loaded(ready) = execute(SceneAction::Import(target.clone())).unwrap() else { panic!("expected scene"); };
    ...
    assert!(execute(SceneAction::Export(broken.clone(), Box::new(manifest), Some(target.join("appearance/appearance.toml")))).is_err());
    ...
    for file in ["appearance/default.png", "appearance/appearance.toml", "scene.toml"] { std::fs::remove_file(target.join(file)).unwrap(); }
