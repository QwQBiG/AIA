...
fn main() -> Result<(), Box<dyn Error>>
{
    ...
    let destination = PathBuf::from(arguments.next().ok_or("usage: create_image_pack NEW_DIRECTORY")?);
    ...
    let poses = ["default", "speaking", "listening", "thinking", "happy", "happy_speaking", "blink"];
...
fn portrait(pose: &str) -> RgbaImage
{
    ...
    for y in 0..320_i32
    {
        for x in 0..256_i32
        {
            ...
            {
                color = if pose == "listening" { [103, 205, 183, 255] } else { [126, 179, 246, 255] };
            }
            ...
            let eyes = [103 + gaze, 153 + gaze].iter().any(|cx|
                (x - cx).abs() <= 4 && (y - 120).abs() <= if pose == "blink" || happy { 2 } else { 9 });
