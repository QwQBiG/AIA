...
async fn edits_keep_history_but_identity_switches_clear_it_and_stop_previous_audio() {
    ...
    handle
        .set_persona("friend".to_owned(), "friend version one".to_owned())
        .await
        .unwrap();
    handle.submit("friend-only history").await.unwrap();
    handle
        .set_persona("friend".to_owned(), "friend version two".to_owned())
        .await
        .unwrap();
    ...
    assert_eq!(
        switched.messages,
        vec![
            Message::new(Role::System, "host identity"),
            Message::new(Role::User, "new scene")
        ]
    );
    handle
        .set_persona("friend".to_owned(), "friend version two".to_owned())
...
async fn failed_scope_switch_preserves_identity_prompt_and_history() {
    ...
    runtime
        .set_persona("default".to_owned(), "original persona".to_owned())
        .await
        .unwrap();
    runtime.run_turn("original history").await.unwrap();
    assert!(
        runtime
            .set_persona("new".to_owned(), "replacement".to_owned())
            .await
            .is_err()
    );
    assert!(
        runtime
            .set_persona(" ".to_owned(), "invalid".to_owned())
            .await
            .is_err()
    );
    ...
    assert!(
        captured
            .messages
            .iter()
            .any(|message| message.content == "original history")
    );
}
...
async fn active_turn_rejects_identity_switch_without_interrupting_the_reply() {
    ...
    assert!(
        handle
            .set_persona("other".to_owned(), "other persona".to_owned())
            .await
            .is_err()
    );
    assert!(!state.speech_interrupted.load(Ordering::Acquire));
