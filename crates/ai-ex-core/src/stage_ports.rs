...
use ai_ex_domain::{
    AppError, Emotion, STAGE_TELEMETRY_SCHEMA_VERSION, StageActionSummary, StageSnapshot, TurnId,
};
...
fn summarize_action(action: &StageAction) -> String {
    match action {
        StageAction::Speak {
            ...
            "turn_id={turn_id:?} text_chars={} interruptible={interruptible}",
            text.chars().count()
        ),
        StageAction::Subtitle { text, duration_ms } => format!(
            "text_chars={} duration_ms={duration_ms}",
            text.chars().count()
        ),
...
impl StageJournal {
    ...
    pub fn with_capabilities(capacity: usize, capabilities: BTreeSet<StageCapability>) -> Self {
...
impl crate::AvatarPort for StageAvatarPort {
    async fn set_subtitle(&mut self, text: String) -> Result<(), AppError> {
        let mut router = self.router.lock().await;
        if !router.capabilities().contains(&StageCapability::Subtitle) {
            return Ok(());
        }
        let action = StageAction::Subtitle {
            duration_ms: subtitle_duration_ms(&text),
            text,
        };
        self.journal.record(&action);
    ...
    async fn interrupt_presentation(&mut self) -> Result<(), AppError> {
        let mut router = self.router.lock().await;
        self.journal.record(&StageAction::Stop);
        router.interrupt_presentation().await
    }
...
mod tests {
    ...
    async fn stage_output_bridges_speech_and_avatar_ports() {
        ...
        avatar.set_speaking(true).await.expect("mouth bridges");
        avatar
            .set_subtitle("hello".to_owned())
            .await
            .expect("subtitle bridges");
        ...
        assert!(
            snapshot
                .actions
                .iter()
                .any(|action| action.kind == "expression")
        );
        ...
        assert!(
            snapshot
                .actions
                .iter()
                .any(|action| action.kind == "subtitle")
        );
        assert!(
            !snapshot
                .actions
                .iter()
                .any(|action| action.detail.contains("hello"))
        );
    }
}
