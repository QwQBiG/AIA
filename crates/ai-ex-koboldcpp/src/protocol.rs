...
pub fn parse_event(line: &[u8]) -> Result<StreamEvent, AppError> {
    ...
    let document: Value = serde_json::from_str(data)
        .map_err(|error| AppError::protocol(format!("invalid KoboldCpp SSE JSON: {error}")))?;
    ...
    let text = document
        ...
        .or_else(|| document.get("results")?.get(0)?.get("text")?.as_str());
    Ok(match text {
...
mod tests {
    ...
    fn parses_token_shapes_and_done_marker() {
        ...
        assert_eq!(
            parse_event(b"data: [DONE]").expect("done"),
            StreamEvent::Done
        );
    }
}
