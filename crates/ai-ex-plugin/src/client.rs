...
use crate::{
    JSON_RPC_VERSION, JsonRpcRequest, JsonRpcResponse, MAX_JSON_RPC_LINE_BYTES, PluginHealth,
    PluginManifest, RpcId,
};
    ...
    W: AsyncWrite + Unpin,
{
    ...
    pub async fn health(&mut self) -> Result<PluginHealth, AppError> {
        decode_result(
            self.request("health", Value::Object(Default::default()))
                .await?,
        )
    }

    pub async fn manifest(&mut self) -> Result<PluginManifest, AppError> {
        let manifest: PluginManifest = decode_result(
            self.request("manifest", Value::Object(Default::default()))
                .await?,
        )?;
        manifest.validate()?;
        Ok(manifest)
...
impl StdioPlugin {
    pub fn spawn(
        ...
        let mut child = command.spawn().map_err(|error| {
            AppError::unavailable(format!("plugin process spawn failed: {error}"))
        })?;
        let stdin = child
            .stdin
            .take()
    ...
    pub async fn shutdown(mut self) -> Result<(), AppError> {
        self.child.kill().await.map_err(|error| {
            AppError::unavailable(format!("plugin process stop failed: {error}"))
        })?;
        self.child.wait().await.map_err(|error| {
            AppError::unavailable(format!("plugin process wait failed: {error}"))
        })?;
        Ok(())
    }
    ...
    pub fn try_wait(&mut self) -> Result<Option<std::process::ExitStatus>, AppError> {
        self.child.try_wait().map_err(|error| {
            AppError::unavailable(format!("plugin process status failed: {error}"))
        })
    }
}
    ...
    R: AsyncBufRead + Unpin,
{
    ...
    let response: JsonRpcResponse = serde_json::from_str(text.trim_end()).map_err(|error| {
        AppError::protocol(format!("invalid plugin JSON-RPC response: {error}"))
    })?;
