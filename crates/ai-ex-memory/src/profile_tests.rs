...
async fn profiles_isolate_recall_export_clear_and_survive_reopening()
{
    ...
    for (profile, fact) in [("friend", "shared clue: tea"), ("host", "shared clue: coffee")]
    {
        store.select_profile(profile).await.unwrap();
        live_writer.remember_kind(MemoryKind::Persona, TurnId::new(), fact.to_owned(), "ok".to_owned()).await.unwrap();
    ...
    assert!(reopened.recall_kind(None, "shared clue", 10).await.unwrap()[0].content.contains("tea"));
...
async fn untagged_records_belong_only_to_default_profile()
{
    ...
    assert_eq!(store.len().await, 1);
    assert_eq!(tokio::fs::read_to_string(&path).await.unwrap(), format!("{legacy}\n"));
...
async fn switching_waits_for_a_pending_write_to_finish_in_its_original_scope()
{
    ...
    {
        writer.remember(TurnId::new(), "original scope".to_owned(), "ok".to_owned()).await.unwrap();
    });
    ...
    let mut switching = tokio::spawn(async move { switcher.select_profile("other").await.unwrap(); });
    assert!(tokio::time::timeout(std::time::Duration::from_millis(20), &mut switching).await.is_err());
    drop(write_guard);
    ...
    let reopened = MemoryStore::open(&path).await.unwrap();
    assert_eq!(reopened.len().await, 1);
    assert!(reopened.recall("original", 10).await.unwrap()[0].content.contains("original scope"));
    tokio::fs::remove_file(path).await.unwrap();
}
