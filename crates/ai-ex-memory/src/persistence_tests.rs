...
async fn populated_store() -> (PathBuf, MemoryStore)
{
    ...
    let mut store = MemoryStore::open(directory.join("memory.jsonl")).await.unwrap();
...
async fn export_rejects_existing_backup_and_source_without_changing_them()
{
    ...
    assert!(store.export_kind(Some(MemoryKind::Viewer), &source).await.is_err());
    ...
    assert!(store.export_kind(Some(MemoryKind::Viewer), &backup).await.is_err());
...
async fn clear_preserves_unrelated_temporary_file_and_can_clear_last_record()
{
    ...
    tokio::fs::write(&unrelated, b"existing recovery data").await.unwrap();
    assert_eq!(store.clear_kind(MemoryKind::Viewer).await.unwrap(), 1);
    assert_eq!(tokio::fs::read(&unrelated).await.unwrap(), b"existing recovery data");
