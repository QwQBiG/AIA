...
use ai_ex_event_bus::{LiveEvent, LiveEventEnvelope, envelope};
    ...
    pub fn validate(&self) -> Result<(), AppError> {
            ...
            || self.reconnect_delay_ms == 0
        {
            return Err(AppError::configuration(
                "invalid bilibili connector settings",
            ));
        }
        Ok(())
    }
        ...
        loop {
            ...
            match result {
                Ok(events) => {
                    self.set_health(
                        true,
                        format!("connected; received {} event(s)", events.len()),
                    );
                    return Ok(events);
                }
                Err(error) => {
...
impl BilibiliConnection {
    pub async fn connect(settings: &BilibiliSettings) -> Result<Self, AppError> {
        settings.validate()?;
        let (mut socket, _response) = connect_async(&settings.endpoint).await.map_err(|error| {
            AppError::connectivity(format!("bilibili websocket connect failed: {error}"))
        })?;
        ...
        socket
            ...
            .await
            .map_err(|error| {
                AppError::connectivity(format!("bilibili handshake failed: {error}"))
            })?;
        Ok(Self {
            socket,
    ...
    pub async fn next_events(&mut self) -> Result<Vec<LiveEventEnvelope>, AppError> {
        ...
        loop {
            ...
            let message = message
                ...
                .map_err(|error| {
                    AppError::connectivity(format!("bilibili websocket read failed: {error}"))
                })?;
            match message {
                Message::Binary(bytes) => {
                    ...
                    for packet in packets {
                        ...
                        let Some(event) = map_message(self.session_id, self.room_id, &packet.body)?
                        else {
                            continue;
                        };
                        events.push(event);
...
pub fn encode_packet(operation: u32, version: u16, body: &[u8]) -> Result<Vec<u8>, AppError> {
    ...
    if total as usize > MAX_PACKET_BYTES {
        return Err(AppError::configuration(
            "bilibili packet exceeds safety limit",
        ));
    }
...
fn decode_packets_at_depth(bytes: &[u8], depth: usize) -> Result<Vec<BilibiliPacket>, AppError> {
    if depth > 4 {
    ...
    while offset < bytes.len() {
        ...
        if header < 16 || total < header || total > bytes.len() - offset || total > MAX_PACKET_BYTES
        {
        ...
        match version {
            ...
            2 => {
                ...
                decoder.read_to_end(&mut decompressed).map_err(|error| {
                    AppError::protocol(format!("bilibili zlib decompression failed: {error}"))
                })?;
                if decompressed.len() > MAX_PACKET_BYTES {
                    return Err(AppError::protocol(
                        "bilibili decompressed packet exceeds safety limit",
                    ));
                }
...
fn map_message(
    ...
    room_id: u64,
    body: &[u8],
) -> Result<Option<LiveEventEnvelope>, AppError> {
    ...
    } else if command == "INTERACT_WORD" && data.get("msg_type").and_then(Value::as_u64) == Some(2)
    {
...
mod tests {
    ...
    fn decodes_nested_zlib_packets() {
        ...
        first_encoder
            .write_all(&inner)
            .expect("first compression writes");
        ...
        second_encoder
            .write_all(&nested)
            .expect("second compression writes");
        let second = second_encoder
            .finish()
            .expect("second compression finishes");
    ...
    fn maps_gift_follow_and_super_chat_events() {
        ...
        .expect("gift maps");
        assert!(
            matches!(gift.payload, LiveEvent::Gift { count: 2, event_id, .. } if event_id == "gift-1")
        );
    ...
    async fn connector_reconnects_and_event_bus_deduplicates_platform_id() {
        ...
        let second = connector
            .next_events()
            .await
            .expect("reconnected event batch");
