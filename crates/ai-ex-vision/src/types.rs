    ...
    pub fn new(media_type: ImageMediaType, bytes: Vec<u8>) -> Result<Self, AppError> {
        ...
        if !signature_valid {
            return Err(AppError::protocol(
                "vision image signature does not match media type",
            ));
        }
...
mod tests {
    ...
    fn validates_image_signature() {
        assert!(
            VisualFrame::new(ImageMediaType::Png, b"\x89PNG\r\n\x1a\ncontent".to_vec(),).is_ok()
        );
    ...
    fn detects_supported_media_without_trusting_extension() {
        let frame = VisualFrame::detect(b"RIFFsizeWEBPcontent".to_vec()).expect("WebP frame");
    ...
    fn rejects_empty_prompt() {
        let frame = VisualFrame::new(ImageMediaType::Jpeg, vec![0xff, 0xd8, 0xff, 0x00])
            .expect("JPEG frame");
