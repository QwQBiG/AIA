    ...
    pub fn from_samples(
            ...
            || !samples.len().is_multiple_of(channels as usize)
        {
            return Err(AppError::protocol(
                "invalid speech PCM format or incomplete frame",
            ));
        }
        if samples.iter().any(|sample| !sample.is_finite()) {
            return Err(AppError::protocol(
                "speech PCM contains a non-finite sample",
            ));
        }
        ...
        let levels = samples
            ...
            .map(|window| {
                ...
                let power = window
                    .iter()
                    .map(|value| f64::from(value.clamp(-1.0, 1.0)).powi(2))
                    .sum::<f64>()
                    / window.len() as f64;
                let rms = power.sqrt();
                if rms <= 0.01 {
                    ...
                    (rms * 4.0 * 1000.0).min(1000.0) as u16
                }
            })
            .collect();
        Ok(Self {
...
mod tests {
    ...
    fn rejects_bad_pcm_and_suppresses_background_noise() {
        ...
        assert_eq!(
            SpeechEnvelope::from_samples(&[0.001; 40], 1, 1000)
                .unwrap()
                .level_at(0),
            0
        );
