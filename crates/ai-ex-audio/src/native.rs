...
impl AudioPlayer
{
    pub async fn play_wav_observed<F>(
        &self, job: &SpeechJob, bytes: Vec<u8>, observer: F,
    ) -> Result<(), AppError>
    where F: Fn(SpeechPlaybackSnapshot) + Send + 'static,
    {
        ...
        tokio::task::spawn_blocking(move ||
        {
            ...
            {
                return Err(AppError::protocol("decoded speech exceeds the PCM sample budget"));
            }
            ...
            Ok(())
        }).await.map_err(|error| AppError::unavailable(error.to_string()))?
    }
}
