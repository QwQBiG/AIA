...
pub(super) async fn complete_turn(service: &ServiceProcess, text: &str) {
    ...
    tokio::time::timeout(Duration::from_secs(3), async {
        loop {
            let ControlPayload::Events(batch) = service
                .client
                .send(ControlCommand::Events { after, limit: 1000 })
                .await
                .unwrap()
            else {
                panic!("events expected");
            };
            if let Some(fault) = batch
                .iter()
                .find(|event| matches!(event.event, SystemEvent::Fault { .. }))
            {
                panic!(
                    "turn failed: {:?}; service log: {}",
                    fault.event,
                    fs::read_to_string(service.directory.join("stderr.log")).unwrap()
                );
            }
            if batch
                .iter()
                .any(|event| matches!(event.event, SystemEvent::TurnFinished { .. }))
            {
                break;
            }
...
async fn service_switches_profile_without_leaking_history_or_recalled_memory() {
    ...
    let server = tokio::spawn(async move {
        loop {
            ...
            let body = if chat {
                sent.send(serde_json::from_slice::<serde_json::Value>(&request).unwrap())
                    .await
                    .unwrap();
    ...
    service
        .client
        .send(ControlCommand::SetPersona { profile: host })
        .await
        .unwrap();
    complete_turn(&service, "shared clue").await;
    let request = captured.recv().await.unwrap().to_string();
    ...
    let mut config =
        ai_ex_config::AppConfig::parse(&fs::read_to_string(&config_path).unwrap()).unwrap();
    ...
    assert!(request.contains("host-only coffee"));
    assert!(!request.contains("default-only tea"));
    let ControlPayload::Persona(profile) =
        service.client.send(ControlCommand::Persona).await.unwrap()
    else {
        panic!("persona expected");
    };
