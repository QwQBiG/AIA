...
async fn parent_closed() -> Result<(), AppError> {
    ...
    std::thread::Builder::new()
        .name("ai-ex-parent-lifetime".to_owned())
        .spawn(move || {
            ...
            let result = loop {
                match input.read(&mut buffer) {
                    ...
                    Err(error) => {
                        break Err(AppError::unavailable(format!(
                            "parent pipe failed: {error}"
                        )));
                    }
                }
            };
    ...
    receiver
        .await
        .map_err(|_| AppError::unavailable("parent lifetime watcher stopped"))?
}
