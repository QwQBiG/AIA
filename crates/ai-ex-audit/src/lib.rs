...
impl AuditSink for JsonlAuditLog {
    async fn record(&mut self, record: AuditRecord) -> Result<(), AppError> {
        ...
        self.file.write_all(&bytes).await.map_err(|error| {
            AppError::unavailable(format!("cannot write audit record: {error}"))
        })?;
        self.file.flush().await.map_err(|error| {
            AppError::unavailable(format!("cannot flush audit record: {error}"))
        })?;
        self.file
            .sync_data()
            .await
    ...
    for (index, line) in content.lines().enumerate() {
        ...
        serde_json::from_str::<AuditRecord>(line).map_err(|error| {
            AppError::protocol(format!(
                "invalid audit record at line {}: {error}",
                index + 1
            ))
        })?;
    }
...
mod tests {
    ...
    async fn persists_and_reopens_a_valid_log() {
        ...
        JsonlAuditLog::open(&path)
            .await
            .expect("reopen valid audit");
