...
impl SceneManifest
{
    ...
    {
        if self.schema_version != 1 || self.id.is_empty() || self.id.len() > 128
            || !self.id.bytes().all(|byte| byte.is_ascii_alphanumeric() || b"._-".contains(&byte))
            || self.name.trim().is_empty() || self.name.chars().count() > 128
        {
            return Err(AppError::configuration("invalid scene identity or unsupported schema_version; expected 1"));
        }
        self.character.to_toml()?;
        if !self.appearance.scale.is_finite() || !(0.25..=1.0).contains(&self.appearance.scale)
        {
            return Err(AppError::configuration("scene appearance scale must be between 0.25 and 1"));
        }
        match (&self.appearance.body, &self.appearance.package)
        {
            (SceneBody::Images, Some(path)) if valid_relative(path) => {}
            (SceneBody::Images, _) => return Err(AppError::configuration("image scenes require a relative appearance package")),
            (_, Some(_)) => return Err(AppError::configuration("only image scenes may reference an appearance package")),
            _ => {}
        }
        Ok(())
    }
...
{
    !path.is_empty() && path.len() <= 512 && !path.contains(['\\', ':', '\0'])
