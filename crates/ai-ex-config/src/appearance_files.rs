...
impl LoadedAppearance {
    pub fn load(path: &Path) -> Result<Self, AppError> {
        ...
        let root = source
            .parent()
            .ok_or_else(|| AppError::configuration("appearance manifest needs a directory"))?;
        ...
        for (key, relative) in &manifest.images {
            ...
            let path = candidate
                .canonicalize()
                .map_err(|error| failure(&candidate, error))?;
            if !path.starts_with(root) {
                return Err(AppError::configuration(format!(
                    "image {key} resolves outside its appearance directory"
                )));
            }
            ...
            if total > 32 * 1024 * 1024 {
                return Err(AppError::configuration(
                    "appearance images exceed the 32 MiB package limit",
                ));
            }
...
fn read_limited(path: &Path, limit: usize) -> Result<Vec<u8>, AppError> {
    ...
    {
        return Err(AppError::configuration(format!(
            "appearance resource is not a file: {}",
            path.display()
        )));
    }
    ...
    if bytes.len() > limit {
        return Err(AppError::configuration(format!(
            "appearance resource exceeds {limit} bytes: {}",
            path.display()
        )));
    }
    Ok(bytes)
...
fn failure(path: &Path, error: std::io::Error) -> AppError {
    AppError::configuration(format!(
        "cannot read appearance resource {}: {error}",
        path.display()
    ))
}
