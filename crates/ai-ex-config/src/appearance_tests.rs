...
fn validates_version_paths_and_required_default() {
    ...
    assert!(
        AppearanceManifest::parse(&manifest("default = 'face.png'\nunknown = 'x.png'")).is_err()
    );
    assert!(
        AppearanceManifest::parse(
            &manifest("default = 'face.png'").replace("version = 1", "version = 2")
        )
        .is_err()
    );
    for path in [
        ...
        assert!(
            AppearanceManifest::parse(&manifest(&format!("default = '{path}'"))).is_err(),
            "{path}"
        );
    }
}
...
fn selects_measured_mouth_and_emotion_with_optional_frame_fallbacks() {
    ...
    assert_eq!(
        pack.frame_key(ConversationState::Speaking, Emotion::Happy, true, false),
        "happy_speaking"
    );
    assert_eq!(
        pack.frame_key(ConversationState::Speaking, Emotion::Happy, false, false),
        "happy"
    );
    assert_eq!(
        pack.frame_key(ConversationState::Speaking, Emotion::Sad, true, false),
        "speaking"
    );
    assert_eq!(
        pack.frame_key(ConversationState::Idle, Emotion::Neutral, false, true),
        "blink"
    );
    assert_eq!(
        pack.frame_key(ConversationState::Listening, Emotion::Happy, false, false),
        "listening"
    );
    for state in [
    ...
    assert_eq!(
        minimal.frame_key(ConversationState::Speaking, Emotion::Happy, true, false),
        "default"
    );
}
...
fn loads_directory_and_rejects_missing_or_oversized_assets() {
    ...
    fs::write(
        root.join("appearance.toml"),
        manifest("default = 'face.png'"),
    )
    .unwrap();
    assert!(LoadedAppearance::load(&root).is_err());
    fs::write(
        root.join("face.png"),
        b"fixture; decoder validation belongs to the renderer",
    )
    .unwrap();
    let loaded = LoadedAppearance::load(&root).unwrap();
    assert_eq!(loaded.manifest.name, "小伙伴");
    assert!(loaded.source.is_absolute());
    fs::File::create(root.join("face.png"))
        .unwrap()
        .set_len(4 * 1024 * 1024 + 1)
        .unwrap();
    assert!(
        LoadedAppearance::load(&root)
            .err()
