...
impl CharacterManifest {
    ...
    pub fn validate(&self) -> Result<(), AppError> {
        if self.schema_version != 1 {
            return Err(AppError::configuration(
                "unsupported character schema_version; expected 1",
            ));
        }
        self.persona.validate()?;
        if self.author.chars().count() > 512
            || self.license.chars().count() > 1024
            || self.persona.taboos.len() > 64
            || self.persona.compiled_system_prompt().chars().count() > 16_384
        {
            return Err(AppError::configuration(
                "character metadata or compiled prompt exceeds supported bounds",
            ));
        }
        Ok(())
    }
            ...
            for key in persona.keys() {
                ...
                .contains(&key.as_str())
                {
                    return Err(AppError::configuration(format!(
                        "unsupported character persona field: {key}"
                    )));
                }
            }
        }
    ...
    pub fn load(path: &Path) -> Result<Self, AppError> {
        ...
        if !file.metadata().map_err(failure)?.is_file() {
            return Err(AppError::configuration(
                "character manifest must be a regular file",
            ));
        }
