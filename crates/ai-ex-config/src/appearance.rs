...
impl AppearanceManifest {
    pub fn parse(text: &str) -> Result<Self, AppError> {
        if text.len() > 65_536 {
            return Err(AppError::configuration(
                "appearance manifest exceeds 64 KiB",
            ));
        }
        let manifest: Self = toml::from_str(text).map_err(|error| {
            AppError::configuration(format!("invalid appearance manifest: {error}"))
        })?;
        manifest.validate()?;
        Ok(manifest)
    }
    ...
    pub fn validate(&self) -> Result<(), AppError> {
        if self.schema_version != APPEARANCE_SCHEMA_VERSION {
            return Err(AppError::configuration(
                "unsupported appearance schema_version; expected 1",
            ));
        }
        if self.id.is_empty()
            || self.id.len() > 128
            ...
            || self.author.chars().count() > 512
            || self.license.chars().count() > 1024
        {
            return Err(AppError::configuration(
                "appearance identity is outside supported bounds",
            ));
        }
        if !self.images.contains_key("default") || self.images.len() > 24 {
            return Err(AppError::configuration(
                "appearance requires a default image and at most 24 frames",
            ));
        }
        for (key, path) in &self.images {
            let known = matches!(
                key.as_str(),
                "default" | "listening" | "thinking" | "speaking" | "blink"
            ) || ["happy", "sad", "angry", "surprised"]
                .iter()
                .any(|emotion| {
                    key == emotion
                        || key == &format!("{emotion}_speaking")
                        || key == &format!("{emotion}_blink")
                });
            if !known {
                return Err(AppError::configuration(format!(
                    "unsupported appearance image key: {key}"
                )));
            }
            if path.len() > 512
                || path.contains(['\\', ':', '\0'])
            ...
            {
                return Err(AppError::configuration(format!(
                    "image {key} requires a relative PNG/JPEG path using forward slashes"
                )));
            }
        }
        Ok(())
    ...
    pub fn frame_key(
        ...
        if matches!(
            ...
            candidates.push(
                if activity == ConversationState::Listening {
                    "listening"
                } else {
                    "thinking"
                }
