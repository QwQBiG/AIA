...
impl SceneBundle
{
    ...
    {
        let path = if path.is_dir() { path.join("scene.toml") } else { path.to_owned() };
        ...
        file.take(131_073).read_to_string(&mut text).map_err(failure)?;
        let manifest = SceneManifest::parse(&text)?;
        let root = source.parent().ok_or_else(|| failure("scene needs a directory"))?;
        let appearance = match &manifest.appearance.package
        {
            Some(relative) =>
            {
                ...
                {
                    return Err(failure("appearance package resolves outside the scene directory"));
                }
                Some(LoadedAppearance::load(&path)?)
            }
            None => None,
        };
        Ok(Self { source, manifest, appearance })
    }

    pub fn save_new(path: &Path, manifest: &SceneManifest, appearance: Option<&LoadedAppearance>) -> Result<PathBuf, AppError>
    {
        manifest.validate()?;
        ...
        if let Some(loaded) = appearance
        {
            ...
            {
                let bytes = loaded.images.get(key).ok_or_else(|| failure("missing image bytes"))?;
                total += bytes.len();
                if bytes.len() > 4 * 1024 * 1024 || total > 32 * 1024 * 1024
                {
                    return Err(failure("scene image resources exceed supported bounds"));
                }
                let extension = relative.rsplit('.').next().ok_or_else(|| failure("image needs an extension"))?;
...
impl BundleWriter
{
    fn new(path: &Path) -> Result<Self, AppError>
    {
        ...
        Ok(Self { directories: vec![root.clone()], root, files: Vec::new(), committed: false })
    }
...
impl Drop for BundleWriter
{
    fn drop(&mut self)
    {
        if !self.committed
        {
            for path in self.files.iter().rev() { let _ignored = fs::remove_file(path); }
            for path in self.directories.iter().rev() { let _ignored = fs::remove_dir(path); }
        }
    }
}
