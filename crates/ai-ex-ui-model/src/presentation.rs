...
impl PresentationState
{
    pub fn from_ui(ui: &UiState) -> Self
    {
        ...
        let expressive = synchronized && !matches!(
            activity, ConversationState::Stopped | ConversationState::Failed | ConversationState::Interrupted,
        );
        Self {
            connected,
            synchronized,
    ...
    {
        let active = self.connected && self.synchronized && !matches!(
            self.activity, ConversationState::Stopped | ConversationState::Failed | ConversationState::Interrupted,
        );
        if !active || reduced_motion || !elapsed_seconds.is_finite()
        {
            return AnimationFrame { breath: 0.0, eyes_open: 1.0, mouth_open: 0.0, gaze_x: 0.0 };
        }
        let time = elapsed_seconds.rem_euclid(120.0) as f32;
        let blink = time.rem_euclid(4.7);
        AnimationFrame {
            breath: (time * 1.6).sin(),
            eyes_open: if blink < 0.16 { (blink / 0.08 - 1.0).abs() } else { 1.0 },
            mouth_open: self.mouth_level.map(|level| level.min(1000) as f32 / 1000.0).unwrap_or_else(||
            {
                if self.activity == ConversationState::Speaking { 0.2 + 0.6 * (time * 9.0).sin().abs() } else { 0.0 }
            }),
            gaze_x: if self.activity == ConversationState::Thinking { 0.6 } else { (time * 0.4).sin() * 0.18 },
        }
    }
}
...
mod tests
{
    ...
    fn disconnect_and_event_gap_do_not_keep_a_stale_expression_alive()
    {
        ...
        assert_eq!(PresentationState::from_ui(&ui).animate(1.0, false).mouth_open, 0.0);
    }
    ...
    fn animation_is_bounded_and_stops_for_interrupt_or_reduced_motion()
    {
        let mut state = PresentationState {
            ...
            activity: ConversationState::Speaking, emotion: Emotion::Happy,
            mouth_level: None,
        };
    ...
    fn model_completion_does_not_end_playback_but_silence_and_interrupt_close_the_mouth()
    {
        ...
        for (index, event) in [
            ...
            SystemEvent::TurnFinished { turn_id, full_text: "hello back".to_owned() },
        ].into_iter().enumerate()
        {
            ui.apply_event(SequencedEvent { sequence: index as u64 + 1, event });
        }
        assert_eq!(PresentationState::from_ui(&ui).activity, ConversationState::Speaking);
        assert_eq!(PresentationState::from_ui(&ui).animate(1.0, false).mouth_open, 0.75);
