...
fn sentence_timeline_replays_resynchronizes_and_rejects_cancelled_or_previous_speech() {
    ...
    apply(SystemEvent::TurnStarted {
        turn_id,
        user_text: "hi".to_owned(),
    });
    apply(SystemEvent::EmotionChanged {
        turn_id,
        emotion: Emotion::Sad,
    });
    apply(SystemEvent::TurnFinished {
        turn_id,
    ...
    apply(SystemEvent::SpeechPlayback {
        playback: playing.clone(),
    });
    apply(SystemEvent::SpeechProgress {
        sentence_id,
        position_ms: 400,
        mouth_level: 500,
    });
    ...
    hub.publish_now(SystemEvent::SpeechProgress {
        sentence_id,
        position_ms: 800,
        mouth_level: 900,
    });
    ...
    let next = TurnId::new();
    hub.publish_now(SystemEvent::TurnStarted {
        turn_id: next,
